// WeShare AI Language Switching Script (FULL VERSION) // Generates window._weshareLangMap, applies DOM translations after button.js rendering (function () {
 ...
 const stringsToTranslate = [ 'Search for social sharing networks', 'Copy Link', 'Post to your blog', 'Follow', 'Share via Email', 'Share to any service' ];
 const translatedMap = {
}
;
 let completed = 0;
...
;
 function applyWeShareTranslations() {
 if (typeof window._weshareLangMap !== 'object') return;
 const elements = document.querySelectorAll('[data-weshare-i18n]');
 elements.forEach(el => {
 const key = el.getAttribute('data-weshare-i18n');
 const translation = window._weshareLangMap[key];
 if (!translation) return;
 if (el.hasAttribute('title')) el.setAttribute('title', translation);
 if (el.placeholder) el.placeholder = translation;
 if (el.classList.contains('copy-text') || el.tagName === 'DIV' || el.tagName === 'SPAN') {
 ...
 setTimeout(applyWeShareTranslations, 50);
 // Wait briefly to ensure DOM exists }
 function fetchTranslation(text) {
 const encodedText = encodeURIComponent(text);
 fetch(`https://weshare-ai-language.vercel.app/api/translate?lang=${
finalLang}
&text=${
encodedText}
